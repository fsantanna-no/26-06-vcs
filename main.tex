...
Note that some rules of the protocol depend on durations of physical time:
    the 12h refund (\R{2}),
...
%
\begin{description}
    \item[Lower limit:] timestamp must be newer than all of its parents.
    \item[Upper limit:] timestamp must be older than the local peer clock.
\end{description}
%
...
\code{b2}), which tolerate a tip older than its parent (\code{tip} vs
\code{b2}).
%
The dashed shapes represent new actions with possibile timings and outcomes in
relation to the tolerance window.
%
The lower limit (\code{parents-1h}) depends only on the self-declared
timestamps of the tip (max among parents) and action, meaning that the
validation is deterministic across peers.
%
...
%
In summary, as in Bitcoin~\cite{btc.timestamps}, initial admission is local,
but all further decisions are deterministic based on the chain itself.
...
, so that members cannot backdate actions before what they acknowledge;
- exemplo do 24h refund

- pq peers nao divergem (unica possibilidade é b, mas é só esperar um pouquinho)


We share with Bitcoin~\cite{TODO}

possible to go back in time


\FC relies on a for that: each action carries a

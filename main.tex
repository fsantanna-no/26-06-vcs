...
%
(ii) \emph{Dave} begs a post (ln~7--8), which lives off-chain (ln~9--10),
waiting \emph{Bob} to like it with \nreps{4000} (ln~11), which after the tax is
split with the author (\R{3.b}), yielding \nreps{1800} each (ln~12--16).
%
...
\footnote{
    Each beg is a root reference in Git, which can be synchronized or dropped
    at will.
    Once liked, it is merged and becomes part of the immutable chain DAG.
}.

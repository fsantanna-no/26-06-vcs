...
termination~\cite{p2p.crdts,p2p.sec}.
%
The last two hold by construction:
    (ii) the DAG only grows by unions, which is commutative, associative, and
    idempotent, leading to monotonic convergence;
    (iii) chain time duration, \reps accounting, and consensus order are pure
    evaluations of the DAG.
%
\FC deliberately gives up eventual delivery (i) when settling windows result
in hard forks (\S\ref{sec.design.partitions.hardforks}).

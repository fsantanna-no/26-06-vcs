...
%
Peers that receive branches with more reputation first just append the other
branch, with no reorderings and no extra efforts.
This behavior is expected to happen in the majority of the connected network.
%
However, the other side needs to reorder all posts after the forking point,
...
%
Regarding content erasure, legitimate posts require members to repost them.
...
However, the longer a peer remains disconnected, the higher are the chances of
conflicting actions.
%
In summary, forks and merges
    (i) may introduce conflicts, which
    (ii) require consensus to replay actions,
    (iii) is based on common history reputation,
    (iv) leading to a global and deterministic state.

...
common ancestors of the DAG, which constitute the forum history which members
...
%
Then, the winning branch is entirely sorted first, with the losing side going
...
Figure~\ref{fig.reps}.A illustrates a forum during a temporary network
partition that creates a fork in its DAG.
...
then all remaining actions are rejected and erased from the primary DAG, as if
they never existed.
%
...
The following recaps the consensus mechanism, and how it applies to
Figure~\ref{fig.reps}:
%
\begin{itemize}
\item Disjoint sets on fork points:         \\
    $SET1=\{c,x,y\}$ and $SET2=\{a,b\}$
\item Sum \reps in common history:          \\
    $SUM1=6$ and $SUM2=8$
\item Order actions with highest first:     \\
    \code{common <- branch-2 <- branch-1}
\item Discard actions after first failure:  \\
    \code{a <- b <- ... <- c <- x} (discards \code{c <- y <- x})
\end{itemize}

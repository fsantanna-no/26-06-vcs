...
%
\begin{itemize}
\item Section~\ref{sec.consensus.design}
    describes the overall reputation and consensus mechanisms, which apply
    to any system based on Merkle DAGs.
\item Section~\ref{sec.consensus.chains}
    describes the concrete reputation rules for public forums in \FC.
\item Section~\ref{sec.consensus.revoke}
    describes how members revoke abusive content.
\item Section~\ref{sec.consensus.algo}
    details the consensus algorithm that orders posts.
\item Section~\ref{sec.consensus.byz}
...
\caption{General reputation rules in \FC.}
\label{fig.general}
\end{table}
...
positively;
a \code{(dis)like} helps members to distinguish content amid excess;
a \code{(un)revoke} toggles content removal.
...
pioneers (each holding 25000 \reps), whose public keys are part of the genesis
commit.
...
In our design, therefore, permissionless dois not that any user can...,
but that any existing insider, on its own, can welcome any outsider without
asking  permissions
any insider can transfer `reps` to welcome any outsider.

TODO: table with new commands

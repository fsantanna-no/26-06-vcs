...
\caption{
    ...
    (A) Single head pointing to genesis post.
    (B) Fork with heads pointing to genesis post.
...
The genesis post with height $0$ and hash \code{42209B..}
depends only on the arguments given to \code{join}.
The next post with height $1$ and hash \code{EF5DE3..} contains the posted
message.
%As expected from a Merkle~DAG, the hash of a post depends on its payload and

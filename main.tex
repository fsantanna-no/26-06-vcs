...
until they reach all peers~\cite{p2p.survey,p2p.byz}, but this does not
guarantee consensus since posts can be received in conflicting
...
networks~\cite{permissionless,blockchain.wild}, which decentralize member
admission and therefore must combat Sybils~\cite{p2p.sybil,p2p.mastodon}.
%
To that end, reputation systems are a well-studied means to build trust among
strangers~\cite{rep.systems}, which we adopt as a core mechanism to also rate
and highlight content, in support of item (iii).

% SZT: o paragrafo com "partially abdicate" foi inteiramente removido e vai ser
% readaptado em Related Work. O argumento é que muitos protocolos sequer tentam
% ter consenso (partially abdicate permissionless consensus)

Bitcoin~\cite{p2p.bitcoin} achieves permissionless
consensus~\cite{permissionless} by demanding a scarce resource---the
\emph{proof-of-work}~\cite{hashcash}---to write to its unique timeline, thereby
resisting Sybils.
%
...
% SZT: strong eventual consistency, sim termo tecnico, e é visto como bom
...
% SZT: nao ha nenhuma tentativa de sincronizacao de clocks
...
% SZT: tem uma secao inteira "Concurrency and Double-spends". o problema do
% double-spend do Bitcoin é bastante conhecido. Mas aqui pode pensar como o
% problema classico de dois writes concorrentes e conflitantes.
...
% SZT: o argumento do mining do Bitcoin é esse: muito mais caro minerar blocos
% do que valida-los.
...
one), each with an independent timeline with its own subjective authoring
etiquette.
% SZT: acho que é cedo pra entender os detalhes. o ponto aqui é que cada forum
% se defende dos outros, pois nao depende de um recurso externo (que poderia
% ser usado em qq forum)
...
% SZT: "familiar and solid p2p foundation"
We implement \FC on top of Git as a familiar and solid peer-to-peer foundation,
providing content addressing, integrity, versioning, signing, deduplication,
compression, and replication for free.
...
mechanisms, discuss relevant security threats, and prove its SEC property
(TODO).
...
% SZT: SEC nao garante tempo nenhum, em p2p os nos podem nunca retornarem.
% eventual é nesse sentido de que qd existe a comunicacao, eles convergem
% sempre de forma monotonica (nao se afastam)
...
% SZT: sync "manual" no sentido de que é par a par, mas pode ter um servico que
% automtiza o processo. somente os pares interessados precisam estar conectados
% nao existe nenhum "hub" importante ou necessario para a rede funcionar
...
%
% SZT: "(not trusted...)"
The action metadata includes its type, the member's timestamp at creation (not
trusted by the protocol), and a reference to the payload blob.
...
order actions because they are unverifiable.

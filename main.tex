...
However, CRDTs are too restrictive and cannot express forum moderation,
since it depends on ordering to prevent double-spends
(\S\ref{sec.design.double} and \S\ref{sec.design.consensus}).
%
In addition, CRDTs alone cannot address content volume or quality under
permissionless access, assuming honest replicas
only~\cite{p2p.merkle-crdts,p2p.byz}.
%
\FC addresses both with reputation-based consensus, which orders the DAG at
the cost of rollbacks (\S\ref{sec.design.consensus}).

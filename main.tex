...
As a countermeasure, branches older than a threshold (7 days) are rejected by
the network, while newer malicious branches may require manual intervention,
but of only a few honest members.
...
\begin{itemize}
    \item fork point within 7 days:
        consensus applies as in \S\ref{sec.design.consensus}.
    \item fork point older than 7 days:
        \code{j} is rejected, regardless of \code{i} or \code{j} winning
        (hard fork rule).
\end{itemize}

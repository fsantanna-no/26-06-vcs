...
%
This criterion resembles \emph{proof-of-stake}~\cite{pos.ouroboros,pos.weak},
but instead of favoring wealth, we reward authors for adding content, which is
valuable in itself.

The consensus outcome, which relies on a deterministic function of the common
history, is a total order of all actions in the DAG.
Peers holding the same branches compute it locally in the same way, without
further votes or coordination.
%
When faced with a fork, the key detail is to account for only the
...
%
Finally, the winning branch is sorted first and the losing one appended
after it, with nested and multi-way forks reducing to successive pairwise

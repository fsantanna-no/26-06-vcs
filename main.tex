...
The DAG encodes the causal relationships between posts, whose cryptographic
links ensure integrity and self certification.
Since the DAG is the full state of a peer, replicas converge by construction,
i.e., the protocol ensures strong eventual consistency
(SEC)~\cite{fed.matrix,p2p.byz}.
...
%
In Table~\ref{tab.commands}, we describe the actual protocol operations of \FC
to guide the discussion through concrete examples.
...
The next sequence of commands simulates a peer \emph{X} that
    (i)   initializes chain \emph{/chat},
    (ii)  posts message \emph{``Hello World''} there, and
    (iii) queries its payload and metadata.
...
The post metadata includes the action type, a local creation timestamp, and a
reference to the payload blob.
Since revocation is a key moderation requirement
(Section~\ref{sec.consensus.revoke}), only the metadata lives in the immutable
...
All other operations except synchronization only affect the local replica.
...
For instance, the \emph{/chat} chain repository contains two commits with
    (1) a genesis identification, and
...
\begin{verbatim}
 ...
 726e329 genesis 0.20.0 5596915440
\end{verbatim}
}
...
In local-first applications, each replica may evolve in separate and
synchronize at some point in the future.
In the example, after the initial \code{clone}, each peer posts in separate and
eventually \code{sync} with the other side, both converging to the same forked
DAG:
...
Naturally, in permissionless forums, we cannot rely on local timestamps because
malicious users could manipulate them.

Another aspect that Merkle DAGs alone cannot solve is Sybil resistance.
Without extra restrictions, any user at any peer could flood chains with
excessive or abusive posts.
%
For instance, CRDT-based systems~\cite{p2p.merkle-crdts,p2p.byz} assume honest
replicas or only address integrity, not volume or content quality, while
federated protocols explicitly leave SPAM filtering to the server

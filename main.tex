...
The total of \nreps{50K} (50 thousand reputation tokens) is equally split and
each pioneer starts with \nreps{25K} (\R{1.a}).
...
Some rules of the protocol depend on durations of physical time:
    the 12h refund (\R{2}),
    the 24h reward (\R{1.b}), and
    the 7-day settling window (\S\ref{sec.design.partitions}).

To measure durations, \FC adopts the principle of a \emph{chain time} relying
exclusively on immutable timestamps already in the chain.
...
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{times.png}
\caption{
...
Therefore, just before the action enters the DAG, the peer validates the
timestamp within reasonable bounds (1-hour tolerance):
%
\begin{description}
    \item[Lower limit:] may not precede parents by more than 1 hour.
    \item[Upper limit:] may not exceed local clock by more than 1 hour.
\end{description}
...
\code{b2}), tolerating a tip slightly older than its parent (\code{tip} vs
\code{b2}).
%
The dashed shapes represent new actions with possible timings and outcomes in
relation to the tolerance window.
%
...
Nevertheless, a peer whose clock lags behind simply refuses the sync as
\emph{too new} until a later retry, and then converges to the same state.

In summary, as in Bitcoin~\cite{btc.timestamps}, initial admission is a local
decision, but once in the chain, decisions are deterministic based on immutable
parents, hence \emph{chain time}.

\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{monotonic.png}
\caption{
...
We also rely on chain time to make the consensus timeline a monotonic clock.
%
Each action in the order carries a \emph{position timestamp}~($P_i$) as the
maximum between the previous position timestamp~($P_{i-1}$) and its own
...
clock.
Note how timestamp \code{4} in the losing branch becomes \code{12} when
preceded by the winning branch.




Figure~\ref{fig.monoti


 by a
temporary network partition. The common ances-
tors have signed actions from members a, b, and c.
...
- also deterministic
 ...
 increase
monotonically, i.e., from the 


 the \emph{consensus time} of an action is the newest
  timestamp up to its position in the order
  (\S\ref{sec.design.consensus}),
  which may be much later than its own, since a losing branch is
  appended
  after the winner regardless of its dates.
  For instance, a post declared three days ago in a losing branch is
  appended
  today: its 24-hour reward is due, since its timestamp is old, but
  its
  consensus time is today, as for the whole appended branch.
  The settling window counts consensus time, so the branch stays
  loose for 7
  days from today, not from its dates.
  Like any chain time, consensus time follows from the DAG alone, so
  peers
  holding the same DAG agree on it.



We also rely on chain time to make the consensus timeline increase
monotonically...
...
Chain time is then the newest timestamp accepted so far, which only advances
as real time passes.

- exemplo do 24h refund
...
Furthermore, the consensus order (\S\ref{sec.design.consensus}) may place an
action far from its declared timestamp, since a losing branch is appended
after the winner regardless of its dates.
We therefore define the \emph{consensus time} of an action as the chain time
at its position in the order, i.e., the maximum between its own timestamp and
the consensus time of the preceding action.
Consensus time follows from the DAG alone, so peers holding the same DAG
compute the same values, and it is what the settling window counts.
...
\item [\S\ref{sec.design.partitions.farming}]
    Reputation Farming:
    how a remote isolated branch mints \reps unchecked.
...
Therefore, \FC protects settled local branches from outdated actions, which
corresponds to checkpoints in proof-of-stake~\cite{pos.weak}.
%
As Figure~\ref{fig.hard} illustrates, actions are frozen 7 days after they
enter the local order, counting in chain time (\S\ref{sec.design.poa}).

 as of
their position in the order, not as of their declared timestamps.
Hence, a losing branch that arrives with old timestamps is appended and remains
loose for 7 days, during which it can still be refuted as in
\S\ref{sec.design.partitions.farming}.
If a winning branch would reorder frozen actions, the merge is refused and the
two peers become incompatible, which is analogous to a hard fork in Bitcoin.
Since consensus time follows from the DAG, peers holding the same DAG reach
the same verdict, and the 7-day window is a chain constant set at creation:
chats may settle faster, newsgroups slower.
%
Peers that remain active and synchronize over time evolve together with a
stable order, whereas a member back after longer must discard its local branch
and repost its actions.
% TODO: 2.13.h
In an idle forum, however, a farmed branch is a fast-forward rather than a
fork, so it enters unchallenged and can only be undone by revoking its posts
(\R{3.d}) or by a deliberate hard fork.

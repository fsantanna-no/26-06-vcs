...
Some rules of the protocol depend on physical clock durations:
    the refund of new posts within 12 hours (\R{2}),
    the reward of consolidated posts after 24 hours (\R{1.b}), and
    the settling window of 7 days (\S\ref{sec.design.partitions}).
%
However, in our permissionless context, we cannot trust on self-declared
timestamps, since malicious members or peers could manipulate them to collect
rewards or simply to vandalize chains.

We share with Bitcoin~\cite{TODO} a \emph{chain time principle}, in which
admission is local, 


is to constrain timestamps within
reasonable bounds
admission is local

Our approach with
Under this principle, admission is local
verything after admission is chain-determined

Although we cannot trust,
we cercamos within reasonable limits

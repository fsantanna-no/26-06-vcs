...
Since the full state of a peer is the immutable DAG, which only grows by
appends (posts) and unions (syncs), replicas converge by construction
...
\item[T1:] The new post costs \nreps{500} (\R{2}), which limits how often
    she can post.
\item[T2:] The cost is refunded within 12 hours (\R{2}), or sooner if other
    members act after it.
...
rather than how old it claims to be.
...
%
The last two hold by construction.
The replay algorithm (\S\ref{sec.design.consensus}) visits each action only
once (iii), using pure DAG evaluations for chain time durations, \reps
accounting, and consensus order (ii).
In addition, the DAG only grows by unions, which are commutative, associative,
and idempotent, leading to monotonic convergence (ii).
%

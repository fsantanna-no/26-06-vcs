...
eradicate Sybils, but in the context of public forums.
We integrate a reputation-based consensus~\cite{fcs.sbseg23} into
\FC~\cite{fcs.sbseg20}, a
permissionless peer-to-peer protocol with strong eventual
consistency~\cite{p2p.crdts,p2p.sec}.
...
which can further be spent to rate other posts with likes and dislikes.
...
Unlike proof-of-work, which is extrinsic, reputation tokens are intrinsic to
each forum.
%
...
Since payloads are only references, peers can prove revocation and refuse to
forward abusive content.
\end{description}

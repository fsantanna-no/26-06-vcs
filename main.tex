...
Even if both branches are legitimate but were genuinely disconnected for a
long period, it is unacceptable that honest members of an active forum are
overridden by a long-absent branch.
%
...
requires (i) eventual delivery, (ii) strong convergence, and (iii)
termination~\cite{p2p.crdts}.
%
The last two hold by construction for the replay algorithm
(\S\ref{sec.design.consensus}):
(ii) uses pure evaluations for chain time durations, \reps accounting, and
consensus order over the DAG, which synchronizes by appends and unions; and
(iii) visits each action only once.
%
However, partitions become permanent (i) when settling windows result in hard
forks (\S\ref{sec.design.partitions.hardforks}).

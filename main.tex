...
\makecell{Expense\\(combat excess)}
        & 2   & new post &
New post charges \sreps{-}{500} temporarily (except begs), refunded within 12h.
\\
\hline
...
(\R{3.b}) after the tax, yielding \nreps{1800} to \emph{Dave} (ln~12--16).
%
...
community must judge by its frequency and relevance, and revoke in any case.
...
In Listing~\ref{lst.rating}, we show the current \reps of \emph{Dave}'s post
(ln 1--2).
...
outcome is $1350$ (ln 5--6).

The economy of \reps induces members to only rate content they care about.
%
On the one hand, since \reps are finite, members must ponder before transfering
them (\R{3.c}).
On the other hand, since \reps are capped (\R{4.b}), they have an incentive to
rate content, which also dilutes their power.
%
...
Even with ratings, chains remain subject to abuse, such as spam,
misinformation, or even illegal content.
For such cases, members may \emph{revoke} an action (\R{3.d}) to erase its
payload in all honest peers.
%
In Listing~\ref{lst.revoke}, \emph{Dave} posts spam (ln~1--2), which
\emph{Alice} revokes (ln~3), making its payload unavailable (ln~4--5).
%
Since actions reference payloads by hash (\S\ref{sec.git}), peers can prove
a revocation and refuse to forward the payload without touching the chain
history, where only the metadata remains.
%
Revocation is reversible through \emph{unrevoke} (ln~6--9), so its availability
follows the balance between revokes and unrevokes (and likes).
Note that a copy of the payload must be provided, which is validated against
its hash persisted in the metadata.
%
As a \emph{right to be forgotten}, authors may revoke their own posts for
...
action, posting spam or rating content from multiple peers at the same time.
As discussed in Section~\ref{sec.git}, concurrency creates forks in the DAG,
which implies ambiguity in the order of actions.

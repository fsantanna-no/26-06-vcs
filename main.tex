...
foundation for local-first concurrent updates, as operations are commutative
...
Automerge~\cite{p2p.automerge} and its peer-to-peer collaboration
apps~\cite{p2p.pushpin} are widely used, but in permissioned settings, where
replicas are trusted collaborators.
%
In fact, we rely on the Git DAG as a state-based CRDT (CvRDT), since it only
grows by appends and unions, so that peers converge by construction

...
The monotonic timeline counts how long an action has been in the local order
rather than how old it claims to be.
This is fundamental to prevent losing branches to settle instantly when
recovering from network partitions (\S\ref{sec.design.partitions}).
Like any chain time, position timestamps follow from the DAG alone, so peers
holding the same DAG agree on them.

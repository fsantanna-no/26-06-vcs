...
% SZT.1: nao ha nenhuma tentativa de sincronizacao de clocks
% SZT.2: tem uma secao inteira "Concurrency and Double-spends". o problema do
...
% SZT.3: o argumento do mining do Bitcoin é esse: muito mais caro minerar blocos
% do que valida-los.
% SZT.4: acho que é cedo pra entender os detalhes. o ponto aqui é que cada forum
...
Members rate and revoke posts as \textbf{decentralized moderation}, which
preserves the Merkle DAG since payloads are referenced by hash.
Since tokens are intrinsic to each forum, unlike proof-of-work, users create
\textbf{independent Sybil-resistant forums}, each with its own timeline and
etiquette.
Since peers need not be online, posts form a causal graph with only a
partial order, which a \textbf{reputation consensus} promotes to a total
order, detecting conflicting operations (akin to Bitcoin's double-spending).

% SZT.5: "familiar and solid p2p foundation"
% TODO: 1.b
...
A \textbf{Git backend} provides content addressing, integrity, versioning,
signing, and replication for free, while non-intrusive hooks add reputation,
consensus, and moderation.

% TODO: 1.b
We also acknowledge three limitations.
Unbounded partitions allow \textbf{long-range attacks} that forge old
branches, as in proof-of-stake~\cite{pos.weak}, so forks older than 100
actions or 7 days are rejected, and newer minority forks require one honest
member to react.
\textbf{Sybil resistance} is not absolute, but each fake identity must pass
proof-of-authoring as a human.
Forum DAGs grow without bound, so \textbf{performance} of storage and
validation degrades over time, as in blockchains, although we simulated
months of a chat and years of a newsgroup from public archives (TODO:
results).

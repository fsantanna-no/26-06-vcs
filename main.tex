...
The design parallels Bitcoin:
    consolidated posts create reputation (cf. proof-of-work),
    likes transfer reputation (cf. transactions), and
    aggregate reputation determines consensus (cf. longest chain).
...
with reputation tokens as a \emph{proof-of-authoring} (akin to mining).
%
...
etiquette.
%
Under network partitions, posts fork the causal graph, and a \emph{reputation
consensus}, resembling proof-of-stake~\cite{pos.ouroboros}, determines a
total order to resolve conflicting operations (akin to double-spending).
...
%
As another limitation shared with blockchains in general, forum graphs are
ever-growing data structures, with costs to store and validate posts that
increase over time.

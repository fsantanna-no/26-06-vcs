...
replays the DAG in topological order, accounts for the transactions in
sequence, detects the conflict, and rejects the second like (ln~17--21).
...
Peers holding the same branches compute it locally in the same way, without
...
cannot persist in the Merkle~DAG.
%
In Figure~\ref{fig.reps}.B, suppose that the last action by $a$ in
\code{branch-2} (in gray) is a dislike to member $c$.
This reduces its \reps and invalidates its last post in \code{branch-1} (in
red), which is rejected together with all actions in sequence.
...
Also, legitimate actions that are erased must be reposted by their authors,
which should be rare, since members are expected to gossip among themselves
often.
...
To measure durations, \FC adopts the principle of a \emph{chain time} that
relies exclusively on immutable timestamps already in the chain.
This way, peers holding the same DAG compute the same durations.
...
%
The lower limit (\code{parents-1h}) depends only on timestamps from immutable
parents already in the chain, meaning that validation is deterministic across
peers.
%
...
Thus, as in Bitcoin~\cite{btc.timestamps}, initial action admission is a
...
We also rely on deterministic chain time to derive a monotonic consensus
timeline.
%
...
%
In Figure~\ref{fig.monotonic}, (A) illustrates a forked DAG with non-monotonic
self-declared timestamps, whereas (B) shows its consensus order as a monotonic
clock.
...
The monotonic timeline counts how new a losing branch is in the local order
rather than how old it claims to be, preventing them from settling instantly
when recovering from network partitions (\S\ref{sec.design.partitions}).
...
stable, especially under a permissionless regime.
Hence, partitions are not exceptional events, but the norm:

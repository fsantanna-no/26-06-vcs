...
Centralized platforms employ moderation policies, but require full trust from
users.
%
...
establish consensus.
As an application example, we prototype a permissionless Wiki that, based on
consensus, resolves conflicts automatically.
\end{abstract}

\noindent\textbf{Keywords:}
Bitcoin, blockchains, CRDTs, distributed consensus, peer-to-peer, public
forums, reputation system, social media, wiki platforms
...
In an ideal public forum, all messages or posts
(i)   reach all users,
...
authority over their own content across machines, but multiple users cannot reach
consensus even in a local machine.
%
Our goal, therefore, is to provide \emph{multi-user/multi-node consensus} (just
\emph{consensus}, from now on) in the context of peer-to-peer public forums.
...
In summary, we propose
    (1) proof-of-authoring as a scarce resource,
    (2) a consensus mechanism based on reputation,
    (3) for independent user-generated blockchains,
    (4) which supports content removal, and
    (5) which can automate conflict resolution in quasi-CRDTs.
%
...
%In addition, they require per-post validations that may become a bottleneck
%for real-time applications.
%
Nevertheless, to show the feasibility of the protocol, we simulated months of
a chat channel and years of a newsgroup forum, both extracted
from publicly available Internet archives.

mostrar tamanhos real vs metas
...
mechanism, discuss relevant security threats, and evaluate its performance.
In Section~\ref{sec.crdts}, we discuss the correspondences with CRDTs, and
prototype a simple conflict-free Wiki.

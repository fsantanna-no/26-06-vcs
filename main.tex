...
the protocol level}~\cite{p2p.dag.sync}, and a requirement to satisfy item
(iii).
Without consensus, each client must rely on local anti-abuse policies, such as
SPAM filters, which only apply \emph{a posteriori}, when the abusive messages
have already been flooded into the network.
%
...
for social content because
    ...
    (c) they impose external economic costs on using the protocol; and
...
%
Another limitation of cryptocurrencies in general is the lack of support for
revoking payloads at the protocol level~\cite{btc.content}, which is
inadmissible considering illicit content in public forums.
...
% TODO: 2.17.b
Reputation systems are a well-studied means to build trust among
strangers~\cite{rep.systems}.
...
causal graph with only a partial order, which we promote to a total order
based on the reputation of members.
...
Users can create diversified forums of interest (rather than a single global
one), each as an independent timeline with its own subjective authoring
etiquette.
...
As a main limitation, forums are DAGs subject to unbounded network partitions,
allowing malicious members to forge long branches, similarly to
\emph{long-range attacks} in proof-of-stake systems~\cite{pos.weak}.
As a countermeasure, forks older than 100 actions or 7 days are rejected by the
network, whereas newer malicious minority forks require manual intervention by
at least one honest member.
Similarly, Sybil resistance is not absolute, but each fake identity must
replicate human behavior to pass proof-of-authoring undetected.
%
...
metadata.
%
The post metadata includes the action type, the member's timestamp at
creation, and a reference to the payload blob.
...
Naturally, in permissionless forums, we cannot rely on members' timestamps to
order posts because they are unverifiable.
...
\hline
Transfer &     & tax      &
All operations burn a 10\% tax before target (member or post).
\\
         & 3.a & like     &
...
A like (\R{3.a}) transfers \reps from the origin to the target, burning a 10\%
...
author: \emph{Bob} spends \nreps{4000}, and \emph{Dave} and their post receive
\nreps{1800} each.
%
...
\item[T2:] The cost is refunded within at most 12 hours (\R{2}), depending on
    the activity following the post.
...
community must detect and dislike in any case.
...
% TODO: 2.17.b
Rating content is a defining feature of public forums, from Slashdot's
scarce moderation points~\cite{forum.slashdot} to Reddit's
votes~\cite{forum.reddit} and Stack Overflow's
reputation~\cite{forum.stackoverflow}.

Likes and dislikes (\R{3.a}) serve three purposes:
    (i)   welcoming new members (\S\ref{sec.design.members}),
    (ii)  rating the quality of posts, and
    (iii) supporting revocation of abusive posts (\R{3.b}).
Quality is subjective and up to members to judge with likes, dislikes, or
abstention.
The \reps of a post is the balance of its likes and dislikes, which end-user
software can use to filter and highlight content.
Since \reps are finite, members must ponder before spending; since \reps are
capped (\R{4.b}), members also have an incentive to spend them on rating.
...
from members holding the majority of the reputation.
% TODO: 2.1.b
These branches are backed by members with more past contributions, which is
analogous to \emph{proof-of-stake}~\cite{pos.ouroboros,pos.weak}, with
reputation as stake, rather than to the longest chain of Bitcoin.
However, proof-of-stake rewards holding, which compounds wealth, whereas
proof-of-authoring rewards authoring, and the cap of \R{4.b} prevents
compounding.
%
When faced with a fork, the key detail is to account for only the reputation in
common ancestors of the DAG, which constitute the forum history that both sides
agree on.
% TODO: 2.1.c
Then, we determine the exclusive members of each side, and compare the total
\reps of these disjoint sets as of the fork point, regardless of how they were
obtained.
% TODO: 2.1.d
Members that act on both sides of the fork are excluded from both sets, since
they contribute equally to either branch, which also neutralizes the
\emph{nothing-at-stake} behavior of proof-of-stake~\cite{pos.weak}, in which
voting on every fork is free.
%
...
then all remaining actions in that branch are rejected and never merged into
the local DAG, as if they never existed.
%
...
% TODO: 2.1.e
    This is subject to \emph{stake grinding}, in which a member varies content
    to obtain a favorable hash, but it only affects ties.
}                                                           \\
...
% TODO: 2.1.g
This settling window plays the role of checkpoints under \emph{weak
subjectivity}~\cite{pos.weak}, in which a peer trusts its own recent view
over any older incoming branch.
...
simply refused and the two peers become incompatible, which is analogous to a
...
The activity threshold counts backwards from the newest local post in the
unstable consensus until older posts are permanently frozen.
...
\begin{itemize}
    % TODO: 2.3.a
    \item The merge is refused if \code{j} wins and would reorder the settled
        prefix of \code{i}, regardless of the reputation of \code{j}.
    \item \code{i} or \code{j} is first, whichever has more reputation in the
        common history (\S\ref{sec.design.consensus}).
    \item Otherwise, branches use the arbitrary criterion of lexicographic
...
blockchain must prevail, forums can split and diverge freely.
...
More than a simple numeric dispute, a hard fork represents a social conflict in
...
positive posts according to Table~\ref{tab.rules}.
%

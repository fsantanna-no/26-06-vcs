...
%
We build \FC on top of Git as a familiar and solid peer-to-peer foundation,
which also provides content addressing, signing, deduplication, compression,
and replication.
%
...
genesis with a random id, and the post pointing to its payload.
...
Note that \code{sync} joins the two tips with a merge commit, so any further
action descends from both sides with the fork closed.
...
%
As another limitation, Merkle DAGs alone cannot provide Sybil resistance in a
permissionless context, since any user at any peer could flood chain DAGs with
abusive posts~\cite{p2p.merkle-crdts,p2p.byz}.
%

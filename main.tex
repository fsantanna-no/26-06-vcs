...
\caption{
    Settled local branch rejecting forks.
}
\label{fig.hard}
\end{figure}
...
%
As Figure~\ref{fig.hard} illustrates, \FC rejects any incoming branch whose
fork point is older than 7 days in chain time (\S\ref{sec.design.poa}), whether
it would win or lose consensus.
...
stable DAG, otherwise they become incompatible similarly to hard forks in
Bitcoin.
A member that stays offline for longer must discard its local branch and
repost its actions.

...
access, but concentrate excessive power, controlling and monetizing over
private user data.
%
Peer-to-peer alternatives~\cite{p2p.survey} eliminate intermediaries, but
strive to achieve consistency when dealing with malicious
...
(a) reach all users,
(b) are delivered in consistent order, and
(c) are respectful and on topic.
In centralized systems, items (a) and (b) are trivially achieved assuming
availability and delivery order, while item (c) requires users to trust the
service policies.
...
protocol level}~\cite{p2p.bec}, and a requirement to satisfy item (c).
...
for social content because
    (a) they encourage a unique global timeline, since forks would dilute value
...
%
For instance, a singleton timeline (a) implies that all forums would be packed
...
In summary, we propose
    (i)   proof-of-authoring as a scarce resource,
    (ii)  a consensus mechanism based on reputation,
    (iii) for independent user-generated blockchains,
    (iv)  which supports content removal, and
    (v)   which can automate conflict resolution in quasi-CRDTs.
%
...
users $a$ and $b$ have abandoned the forum for months, and thus
\code{branch-1} is actually legitimate.
In this case, users $a$ and $b$ would succeed to take over the forum, since
...
users in the network will dislike it until the post becomes
...
protocol, in which users can create public forums of interest and apply diverse
moderation policies.

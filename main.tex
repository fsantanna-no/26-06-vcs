...
We propose two complementary mechanisms:
    ...
    (ii) \emph{begging posts} from newcomers, which wait for members' likes.
...
%
(ii) \emph{Dave} signs a post with \code{--beg} (ln~7--8), which lives
off-chain (ln~9--10).
Then, \emph{Bob} spends \nreps{4000} to like the post (ln~11), which is taxed
and then split with the author (\R{3.b}), yielding \nreps{1800} each
(ln~12--16).
%
...
Section~\ref{sec.design.time} discusses how to count time consistently without
a global clock.
%
...
Hence, we argue that the actual threat is automated content, which the

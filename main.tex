...
cannot sustain a growing chain with new members and posts.
%
...
power.
For public forums, we propose new human-authored content
...
\FC rewards members whose new content persists on the chain (\R{1.b}), which
demands non-trivial work that resists automation.%
\footnote{
    Note that AI-generated content is still subject to human evaluation
    regarding its frequency and relevance.
}
%
...
\item[T1:] The new post costs \nreps{500} (\R{2}), which limits how often
    she can post.
\item[T2:] The cost is refunded within 12 hours (\R{2}), sooner in
    proportion to the \reps of members acting after it.
\item[T3:] The post consolidates and rewards her with \nreps{1000} (\R{1.b}).
\end{description}
%
...
before the reward, while also regulating the growth speed of the chain (since
extra posts do not reward members with more \reps, \R{1.b}).
%
Overall, proof-of-authoring stimulates content creation, which ultimately is
...
%
These are not Sybils in the strict sense, since each identity needs to behave
as an independent human to pass proof-of-authoring.
We argue that the actual threat is automated content, which the community must
detect and revoke in any case.

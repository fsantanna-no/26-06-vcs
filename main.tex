...
\begin{abstract}
Public online forums suffer from and abuse such as SPAM and fake news.
...
conflicting receiving orderings~\cite{p2p.intention,p2p.crdts}.
%
...
SPAM filters), which only apply \emph{a posteriori}, when the damage have
already been flooded into the network.
%
Abuse is particularly acute in \emph{permissionless}
networks~\cite{permissionless,blockchain.wild} with the presence of
Sybils~\cite{p2p.sybil,p2p.mastodon}.
%
To that end, reputation systems are a well-studied means to build trust among
strangers~\cite{rep.systems}, and to rate and highlight content, in support of
item (iii).
...
as a \emph{decentralized moderation} process.
%

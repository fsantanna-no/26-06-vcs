...
foundation for concurrent updates in local-first applications~\cite{p2p.local}.
Operations are designed to commute, so that peers apply them immediately, even
offline, and still converge to the same state.
%
Merkle~DAGs have been proposed as CRDTs that provide strong eventual
consistency and self-certified
archives~\cite{p2p.merkle-crdts,fed.matrix,p2p.byz}, and
Automerge~\cite{p2p.automerge} is a widely used CRDT library for peer-to-peer
collaboration.

However, CRDTs provide no notion of a global order, so conflicts that depend
on ordering still require human intervention, such as manual merges in version
control systems.
Likewise, a DAG has no attached semantics, so applications may interpret it in
conflicting orders.
%
In addition, CRDT systems assume honest replicas or only address
integrity~\cite{p2p.merkle-crdts,p2p.byz}, not volume or content quality.
When DAGs are open to permissionless writes, they are subject to abuse, which
may degrade the content and performance of the network.
%
Since the proposed consensus already relies on human interactions, \FC uses
it to address permissionless writes and to provide a total order for
applications (at the cost of rollbacks).
This order resolves such conflicts automatically.

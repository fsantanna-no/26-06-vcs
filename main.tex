...
for social content because
    (a) they enforce a unique global timeline;
    (b) they lean towards concentration of power~\cite{btc.decentralization};
    (c) they impose external economic costs; and
    (d) they rely on objective rules to reach consensus.
%
A unique timeline that packs all forums under the same objective consensus
rules threatens our decentralization goals and renounces subjectivity, which is
inherent to content moderation.
%
Another limitation of cryptocurrencies is the lack of support for revoking
payloads at the protocol level~\cite{btc.content}, which is inadmissible
considering illicit content in public forums.

% SZT: strong eventual consistency, sim termo tecnico, e é visto como bom
% mas removi daqui pra ser discutido no corpo

In this work, we propose \FC, a permissionless peer-to-peer protocol with
integrated reputation-based consensus~\cite{fcs.sbseg20,fcs.sbseg23}.
We adapt the idea of scarce resources to resist Sybils and reach consensus, but
now in the context of public forums.
We highlight four main contributions:

\textbf{Reputation-based consensus:}
...
\textbf{Independent Sybil-resistant forums:}
...
\textbf{Decentralized moderation:}
...
order actions because they are unverifiable, although we constrain them for
time-based rules (\S\ref{sec.design.poa}).
...
unverifiable (Section~\ref{sec.freechains}).

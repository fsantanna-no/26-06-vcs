...
\hline
\textbf{Rule} & \textbf{\#} & \textbf{Operation} & \textbf{Description} \\
\hline
...
\\
        & 1.b & old post &
Consolidated post grants \sreps{+}{1K} after 24h, at most once a day.
\\
\hline
...
require a welcoming like from any other member already in the system.
%
...
\emph{Bob} likes it with \nreps{4000} (ln~11), which, after the tax, is split
with the author (\R{3.b}), yielding \nreps{1800} each (ln~12--16).
%
Since likes are at best zero-sum (transfers and taxes do not add value), there
is no incentive to cycle \reps among colluding Sybils.

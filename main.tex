...
Our idea to stablish consensus in forum DAGs is to favor branches with posts
from members that constitute the majority of the reputation.
These branches have more associated work and are analogous to longest chains in
Bitcoin.
%
When faced with a fork, the key detail is to account only the reputation in
common ancestors of the DAG, which constitute the forum history wich members
from both sides agree.
Then, we determine the exclusive members of each side, and account the
aggregate reputation of these disjoint sets, but at the time of the fork point.
%
Then, the winning branch is entirely sorted first, with the loosing side going
in sequence, after the last action of the winning side.

\begin{figure*}
\centering
\includegraphics[width=0.85\textwidth]{reps.png}
\caption{
    (A) A forum DAG with common ancestors, a fork point, and two branches.
    (B) The resulting total order after consensus.
}
\label{fig.reps}
\end{figure*}

Figure~\ref{fig.reps}.A illustrates the evolution of a forum DAG in the
presence of a temporary network partition.
...
contributions and therefore hold mode \reps combined than $c$ has alone (i.e.,
$3+5>6$).
%
Then, member $c$ disconnects, and evolves with new members $x$ and $y$ in
\code{branch-1}.
In the meantime, $a$ and $b$ also participate with new posts in
\code{branch-2}.

TODO: esticar dia de c ja que sufix vai sair
...
This transform a partial into a total order, making it possible to 


in its entirely sorted




Then, we employ a deterministic topological sorting that favors reputation
when deciding between branches in a forum DAG, transforming a partial order
into a total order.

...
Therefore, the operation and infrastructure of the protocol are simple:
    (i)  each post is an individual signed commit in the history graph, and
...
%
We demonstrate the actual protocol operations of \FC to guide the discussion
...
\emph{Alice}
    ...
    (iii) posts message \emph{Good Morning!} there, and
...
\begin{verbatim}
 ...
 $ fc chat post 'Good Morning!' --sign=alice
...
Let's now consider a second peer \emph{Y} that replicates \emph{X} through the
\emph{clone} operation:
...
connect to daemon in \emph{remote-ip} to receive all missing posts from there:
...
In this sense, \FC is conceptually a topic-based gossip on how users publish
and consume content, but it still requires extra network automation.

In order to query the state of the replica, the next sequence of commands
checks the hash(es) of the post(s) at the head of the local DAG (the latest
posts), and then reads the payload of the single head found:
...
after synchronizing would now contain two posts with height $1$.
%
...
source timestamps of the posts.

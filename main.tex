...
\begin{lstlisting}[float=tbp,caption={Basic operations of a chain in peer \emph{X}.},label={lst.basics}]
# PEER X
$ fc /chat init
...
\begin{lstlisting}[float=tbp,caption={Internal Git structure of the \emph{/chat} chain.},label={lst.git}]
# PEER X
...
\begin{lstlisting}[float=tbp,caption={Peer \emph{Y} replicates the chain of peer \emph{X}.},label={lst.clone}]
# PEER Y
$ fc /chat clone X
726e32918b7ac760de4088a5959168bde5e5a608
$ fc /chat list dag
...
\begin{lstlisting}[float=tbp,caption={Concurrent posts and synchronization result in a forked DAG.},label={lst.fork}]
# PEER X
$ fc /chat post 'From X'
...
\hline
Emission & Old posts award \reps to authors
         & Encourage content authoring \\
\hline
Expense  & New posts deduct \reps from authors temporarily
         & Discourage excess of content \\
\hline
Transfer & Likes and revokes transfer \reps between authors and to posts
...
peer-to-peer systems, and set two of them as pioneers of a fresh, now
Sybil-resistant \emph{/chat} chain replicated across peers \emph{I} and
\emph{J}:

\begin{lstlisting}[float=tbp,caption={Pioneers bootstrap a Sybil-resistant chain.},label={lst.pioneers}]
$ ssh-keygen -f alice
$ ssh-keygen -f bob
$ ssh-keygen -f silas
...
\hline
Emission    & 1.a & pioneers & Chain creation counts $+$\nreps{50000} equally distributed between the pioneers \\
            & 1.b & old post & Consolidated post counts $+$\nreps{1000} to its author, at most once a day \\
\hline
Expense     & 2   & new post & New post counts $-$\nreps{500} to its author temporarily, refunded within 12 hours \\
\hline
Transfer    & 3.a & like     & Like counts $-$\nreps{n} to origin and $+$\nreps{n} to target, burning a 10\% tax \\
            &     & dislike  & Dislike counts $-$\nreps{n} to origin and $-$\nreps{n} to target \\
            & 3.b & revoke   & Revoke counts $-$\nreps{n} to origin and target, and erases the payload \\
            &     & unrevoke & Unrevoke counts $-$\nreps{n} to origin, and restores the payload \\
\hline
Constraints & 4.a & min      & Author must afford action costs: \nreps{500} to post, \nreps{n} to vote \\
            & 4.b & max      & Author is limited to at most \nreps{50000} \\
            & 4.c & size     & Payloads are limited to at most \emph{128\,KB} (not currently implemented) \\
\hline
\end{tabular}
\caption {
...
refer to throughout the examples, and which are compatible with the general
rules of Table~\ref{fig.general}.
...
\begin{lstlisting}[float=tbp,caption={Welcoming new members with a like and a begging post.},label={lst.welcome}]
$ fc /chat like 5000 author charlie --sign=alice
$ fc /chat reps authors
bob        25000
alice      20000
charlie     4500
...
\subsubsection{Proof-of-Authoring}
...
Therefore, \FC rewards authors that post new content that other members
...
%
To illustrate proof-of-authoring, let us trace how \emph{Charlie}'s reputation
evolves with a post:

\begin{lstlisting}[float=tbp,caption={Proof-of-authoring: reputation of \emph{Charlie} over time.},label={lst.poa}]
$ fc /chat reps author charlie     # T0: before post
4500
$ fc /chat post 'Nice Post' --sign=charlie
1a5c8e21d934e6a44e47f72f188250faeaf0a6de
$ fc /chat reps author charlie     # T1: after post
4000
$ fc /chat reps author charlie     # T2: after 12h
4500
$ fc /chat reps author charlie     # T3: after 24h
5500
\end{lstlisting}

\begin{description}
\item[T0:] \emph{Charlie} holds the \nreps{4500} received from \emph{Alice}'s
...
Since his post survived, \emph{Charlie} ends with \nreps{1000} more than he
...
reward authors with more \reps (\R{1.b}).
%
In summary, proof-of-authoring stimulates content creation, which ultimately is
the \emph{raison d'être} of public forums.

\subsubsection{Double-spend and Consensus}

Scarcity of \reps is not yet sufficient to prevent inconsistent operations,
which correspond to \emph{double-spending} in Bitcoin.
...
In the next example, \emph{Charlie} double-spend most of his \reps with
concurrent likes to \emph{Xion} and \emph{York} on peers \emph{I} and \emph{J},
just before they synchronize.
%
Nevertheless, due to the consensus mechanism we are about to discuss, the peers
refuse the double spend and converge to the same state:

\begin{lstlisting}[float=tbp,caption={\emph{Charlie} double-spends his \reps on peers \emph{I} and \emph{J}.},label={lst.double}]
$ fc /chat reps author charlie
4500

# PEER I
$ fc /chat like 4500 author xion --sign=charlie
...
# PEER J
$ fc /chat like 4500 author york --sign=charlie
...
We assume \emph{Charlie} is honest and online...
Also, local time

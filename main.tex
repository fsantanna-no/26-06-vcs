...
Some rules of the protocol depend on physical clock durations:
    the 12h refund (\R{2}),
    the 24h reward (\R{1.b}), and
    the 7-day settling window to be discussed (\S\ref{sec.design.partitions}).
%
\FC adopts the principle of a \emph{chain time} relying exclusively on
self-declared timestamps already in the chain.
This way, peers in sync sharing the same DAG always compute the same values.
...
to simply vandalize chains.
Therefore, we constrain timestamps within reasonable lower and upper limits as
follows:
>>>
      (a) the \emph{lower limit} is the chain itself:
          an action must be newer than all of its parents,
          so members cannot backdate actions before what they acknowledge;
      (b) the \emph{upper limit} is the local clock of the receiving peer,
          with a tolerance of one hour,
          so members cannot forward-date actions beyond the present of honest
          peers.
...
In summary, initial admission is local, but all further decisions are
deterministic based on the chain itself.

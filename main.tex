...
\title{
    Permissionless Peer-to-Peer Forums on top of Git
}
...
Bitcoin, blockchains, CRDTs, distributed consensus, peer-to-peer,
...
and $c$, when they were still all connected.
We assume that within the prefix, users $a$ and $b$ have contributed with
better content and therefore have more reputation combined than $c$ has alone (i.e.,
$8+5>3$).
%
...
Figure~\ref{fig.reps}.B indicates the resulting consensus order between posts
in the forum.
...
the common suffix.
%
Note that in a Merkle~DAG, it is not possible to remove only the post with the
failing operation, instead, we need to remove the remaining branch completely,
as if it never existed.
%
However, this kind of attack is refutable because, since the common
prefix is the basis of consensus, users in the branch with more reputation can
still react even a posteriori.
...
%
Peers that first receive branches with less reputation need to reorder
all posts after the forking point.
...
\begin{itemize}
    ...
    \item \code{i} or \code{j} is first, whichever has more reputation
          in the common prefix (Section~\ref{sec.consensus.design}).
    \item otherwise, branches use the arbitrary criteria of lexicographical
          order of the post hashes immediately after the
          common prefix.
\end{itemize}

To conclude this section, we should keep in mind that our consensus algorithm
is based on
subjective evaluations of users and posts.
...
to store the complete graph in order to synchronize and verify new posts.
...
In the next sequence of commands, a new remote user joins the same public chain and
...
Finally, the actual contents of a post may be revoked if it has at
...
post becomes \code{REVOKED} and its payload is not retransmitted to other
peers.
...
posts and authors back to some past post in the hard consensus, and disallow
...
%
We executed each simulation $4$ times in a desktop PC (\texttt{i7}

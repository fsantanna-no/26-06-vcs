...
Pioneers address bootstrapping, but a fixed initial supply of \reps (\R{1.a})
cannot sustain a growing chain with new members and posts.
%
Facing the same issue, Bitcoin mints new coins as a reward for CPU power
(\emph{proof-of-work}), which is an \emph{extrinsic resource} not evenly
distributed.
For public forums, we propose human authoring (\emph{proof-of-authoring}) as an
\emph{intrinsic resource}, which is still analogous to mining:

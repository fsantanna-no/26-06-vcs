...
Bitcoin, blockchains, distributed consensus, peer-to-peer, public
forums, reputation system, social media %, CRDTs, Wikis
...
Gossiping protocols proactively disseminate posts among
peers, which does not guarantee consensus due to conflicting delivery
orders~\cite{p2p.intention,p2p.crdts}.
%
...
are a well-studied means to rate and highlight content, and to build trust
among strangers~\cite{rep.systems,rep.eigentrust}.

Bitcoin~\cite{p2p.bitcoin} achieves permissionless
consensus~\cite{permissionless} by demanding a scarce resource
(\emph{proof-of-work}~\cite{hashcash}) to write to its timeline, thereby
resisting Sybils.
%
...
consensus}, resembling \emph{proof-of-stake}~\cite{pos.ouroboros}, determines a
...
\emph{long-range attacks}~\cite{pos.weak}.
...
In Section~\ref{sec.git}, we introduce the basic operations of \FC built on top
of Git, and motivate why permissionless forums require Sybil resistance.

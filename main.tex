...
protocol level}~\cite{p2p.bec}, and a requirement to satisfy item (iii).
...
%
Abuse is particularly acute considering Sybils~\cite{p2p.sybil,p2p.mastodon} in
\emph{permissionless} networks~\cite{permissionless,blockchain.wild}, where
anybody can join with unlimited free identities.

Nevertheless, consensus is notably challenging to the point that decentralized
protocols partially abdicate it along two axes.
%
On the one hand, \emph{multi-user/single-node consensus}, adopted by
federated protocols~\cite{p2p.ecosystem}, supports a consistent multi-user
public forum, but restricted to a single trusted server (ordered by the
server).
On the other hand, \emph{single-user/multi-node consensus}, adopted by
Scuttlebutt~\cite{p2p.scuttlebutt}, supports a consistent message feed
across multiple machines, but restricted to a single author (ordered by the
author).
%
Our goal, therefore, is to support \emph{multi-user/multi-node consensus} in
the context of peer-to-peer public forums, i.e., permissionless counterparts
...
for social content because
    (a) they enforce a unique global timeline, since forks would dilute value
    ...
    (c) they impose an external economic cost to use the protocol; and
...
%
For instance, a unique timeline (a) implies that all forums would be packed
together and subject to the same consensus rules, while objective rules alone
(d) cannot distinguish abuse amid content.
%
...
eradicate Sybils in the context of public forums.
...
strong eventual consistency~\cite{p2p.crdts,p2p.sec}.
...
to create, rate, and synchronize posts.
...
CRDTs~\cite{p2p.crdts}, and prototype a simple conflict-free Wiki.
...
\hline
\code{init}       & Creates a chain with a pioneer author   \\
\code{clone}      & Replicates a chain from a remote peer   \\
\code{sync}       & Exchanges posts with a remote peer      \\
\code{post}       & Appends a post to the chain             \\
\code{payload}    & Queries the post content (revocable)    \\
\code{metadata}   & Queries the post metadata (immutable)   \\
\code{dag}        & Shows the chain DAG                     \\
\hline
\end{tabular}
\caption{Basic \FC operations.}
\label{tab.commands}
\end{table}

\FC~\cite{fcs.sbseg20} is an unstructured peer-to-peer gossip protocol, in
which each topic or \emph{chain} is a replicated
\emph{Merkle Directed Acyclic Graph} (DAG)~\cite{p2p.ipfs}.
%
The DAG represents the causal relationships between posts, whose cryptographic
links ensure persistence and self certification.
Considering that the DAG itself represents the full state of the peers, the
protocol ensures strong eventual consistency (SEC)~\cite{fed.matrix,p2p.byz}.
%
We build \FC on top of Git as a \emph{façade}~\cite{design.patterns} with hooks
to enforce reputation rules and consensus.
Therefore, the operation and infrastructure of the protocol are simple:
    (i)  each post is an individual commit in the history graph, and
    (ii) peers synchronize their full state---the DAGs---through Git tooling.
...
%
In Table~\ref{tab.commands}, we describe the actual protocol operations of \FC to guide
the discussion through concrete examples.

In the next sequence of commands, we simulate a peer \emph{X}, in which
\emph{Alice}
    (i)   creates a key pair to sign operations,
    (ii)  initializes chain \emph{chat} as its pioneer,
    (iii) posts message \emph{Hello World} there, and
    (iv)  query its payload and metadata.

{\footnotesize
\begin{verbatim}
 # PEER X
 $ ssh-keygen -t ed25519 -f alice
 $ fc chat init --pioneer=alice
 #726e32918b7ac760de4088a5959168bde5e5a608
 $ fc chat post 'Hello World' --sign=alice
 ...
 action post
 time 1787850618
 sign ssh-ed25519 ...(alice pubkey)...
 blob 9d882f4f8568a8f79a5d007a576919fae317ad65
\end{verbatim}
}

The post metadata includes the action type, local creation time, author
signature, and a reference to the payload blob.
Since revocation is a key moderation requirement, only the metadata lives in
the immutable chain DAG, but not its payload.
...
chain.
The example chain repository contains two commits:
    a genesis identification with \emph{Alice} as the pioneer, and
    the post pointing to its payload.
...
\begin{verbatim}
 ...
 726e329 id 0.20.0 5596915440 ssh-ed25519 ...(alice)...
\end{verbatim}
}

To illustrate peer-to-peer synchronization, we introduce a second peer \emph{Y}
that replicates \emph{X} through the \emph{clone} operation:
...
Each replica may evolve in separate and synchronize at some point in the future
through the \emph{sync} operation:

{\footnotesize
\begin{verbatim}
 # PEER X
 $ fc chat post 'From X' --sign=alice
 eb18b4527ee5ecc6106a086c35fbfe8c25fa6aba
 ...
 $ fc chat sync Y

 # PEER Y
 $ fc chat post 'From Y' --sign=alice
...
no fim falar de git cvrdt
nao resolve abuso, spam, refernciar sybil/kleppmann

As the example illustrates, chains are not simple lists of sequential posts,
but instead DAGs with multiple heads.
...
However, in forums, a malicious user could modify his local time to
...
As illustrated in Figure~\ref{fig.family}.C, a like is a regular post with an
extra link to its target.
Every new post points to the previous heads, establishing a causal logical
timeline in the chain.
For instance, the JSON that follows represents the post \code{2\_DDA222..}
with the backs and extra like links:
As we discuss in the next section, like operations have to be signed by users,
...
%
In the next section, we propose a consensus mechanism in \FC.
%
...
Without moderation, permissionless peer-to-peer public forums are
impractical due to Sybils abusing the system.
...
In our system, members can spend tokens named \reps to post and rate content in
forums:
...
goals.
To prevent Sybils, members with no \reps cannot operate under these rules,
...
\caption{
    ...
    typical forums with moderate traffic.
...
\begin{itemize}
    ...
    \item \code{i} or \code{j} is first, whichever has more reputation in the
          common prefix (Section~\ref{sec.consensus.design}).
...
\subsection{Experiments with Forum Archives}
...
in Section~\ref{sec.consensus.design}), regardless of the size of the chain,
...
Section~\ref{sec.consensus.eval}).

...
A like transfers \reps from the origin to the target (\R{3.b}), burning a 10\%
...
In \FC, each forum post holds an individual \reps balance as the difference
between likes and dislikes targeting it (\R{1.b}), which can be used in
end-user software can use to filter and highlight content.

TODO: example
- two users, one likes, the other dislikes the same post
- show reps of each user and post

On the one hand, since \reps are finite, members must ponder before spending
them.
On the other hand, since \reps are capped (\R{4.b}), members also have an
incentive to spend them on rating, and thus decentralize power.
%

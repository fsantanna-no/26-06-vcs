...
%
\FC adopts the principle of a \emph{chain time} relying exclusively on
immutable self-declared timestamps already in the chain.
...
to vandalize chains.
Therefore, just before the action enters the DAG (posted or received), the peer
validates the timestamp within reasonable bounds (1-hour tolerance):
%
\begin{description}
...
\end{description}
%
Note that the lower limit depends only on self-declared timestamps with
cryptographic links, meaning that the validation is deterministic.
Therefore, the upper limit is the only source of non-determinism, since each
peer runs its own clock.

...
As a main limitation, because forums are DAGs, members holding the majority of
the reputation at any point in history can forge malicious forks that would
affect the consensus.
As a countermeasure, forks older than 100 posts or 7 days are rejected by the
network, whereas newer malicious majority forks require manual intervention.
%
...
\item [\S\ref{sec.design.double}]
    Forks and Double-spends:
    how concurrency leads to inconsistent operations.
\item [\S\ref{sec.design.consensus}]
    Consensus and Total Order:
    how peers converge to the same accounting of \reps.
\item [\S\ref{sec.design.hardforks}]
    Malicious Behavior and Hard Forks:
    how long partitions split into independent forums.
\end{description}
...
\subsection{Forks and Double-spends}
...
%
Partitions are also \emph{unbounded} and \emph{undetectable}, since a peer may
stay offline for as long as it wants, and one cannot tell whether the network
is unreachable or merely idle.



Partitions are also \emph{unbounded} and \emph{undetectable}, since
  a peer may
  stay offline for as long as it wants, and cannot tell whether the
  network is
  unreachable or merely idle.
  Moreover, since peers keep posting while apart, the divergence to
  reconcile is
  unbounded as well.


, as a peer may stay offline
  for as long as
  it wants.
...
Since \FC is local-first, partitions are also \emph{unbounded}, as a peer may
work offline for as long as it wants.

  Unbounded partitions are, in turn, \emph{undetectable}: a
  partitioned peer
  cannot tell whether the network is unreachable or simply idle.

  Finally, undetectable partitions may be \emph{deliberate}, and no
  peer can tell
  a deliberate partition apart from a legitimate one.
  The protocol, therefore, cannot treat partitions as attacks, but
  only bound the
  damage they may cause.
  %
  In CAP terms~\cite{cap}, \FC favors availability and partition
  tolerance:
  peers always accept local operations and reconcile later, whereas
  the consensus
  order provides a deterministic total order instead of
  linearizability.

  We discuss the two consequences of unbounded partitions:
  \begin{description}
  \item [\S\ref{sec.design.partitions.farming}]
      Farming Reputation: how an isolated branch mints \reps
  unchecked.
  \item [\S\ref{sec.design.partitions.hardforks}]
      Hard Forks: how a settling window bounds the damage.
  \end{description}

  CAP: two sentences is the right dose — enough to position, not a
  lecture. It needs a bib entry (Gilbert & Lynch, "Brewer's
  conjecture and the feasibility of consistent, available,
  partition-tolerant web services", SIGACT News 33(2), 2002); I can
  add it.

  Section list: add the sixth entry to §3's roadmap. One clash to
  resolve — §3.4 is "Forks and Double-spends" and this new one also
  owns forks; renaming §3.4 to "Concurrency and Double-spends" keeps
  a single home for the fork discussion.

  Want me to apply the subsection, the roadmap entry, the rename, and
  the CAP bib?



\subsubsection{Farming Reputation}
...
in p2p with intermittent but unbounded partitions with members authoring at the
same time, dags are inevitable

CAP theorem


If we assume that members can
forks are inevitable
inherent to domain of content authoring

The main limitation of our common-history consensus is that it may trigger at
any fork point in time, no matter how old.
For instance, any arbitrary ancient long partition can reorder an active forum
based on reputation accumulated by former members far in the past.

● Proposed structure for the subsection, same heading.

  Opening paragraph, introducing both cases in general terms:

  Consensus based on the common prefix is robust against members that
  accumulate \reps in isolation, but it does not distinguish a malicious
  partition from a legitimate one.
  We first show how the majority can neutralize an isolated takeover, and then
  why long partitions must be treated as irreconcilable.

  Then the concrete attack by $c$ and its refutation, as in the paragraph proposed
  earlier.

  Transition before "The main limitation...":

  The countermeasure above assumes that the majority is still active and
  willing to react.
  The main limitation of our common-history consensus is that it may trigger at
  any fork point in time, no matter how old.
  For instance, an arbitrarily old partition can reorder an active forum based
  on reputation accumulated by former members far in the past.

  Then the existing "As a counterpoint" paragraph follows naturally, since it is
  exactly the case where the majority is no longer active.
...
The main limitation of common history consensus is that it can trigger at any
fork point in time


Consensus based on the common history has a main limitation


 raises a natural concern: a malicious
member could disconnect on purpose to cultivate fake identities in isolation
and take over the forum after merging.
...
\begin{itemize}
    ...
    \item \code{i} or \code{j} is first, whichever has more reputation in the
          common prefix (\S\ref{sec.design.overall}).

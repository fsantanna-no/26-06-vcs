...
TODO: revocation
\iffalse
  What the revocation paragraph must cover, from Table 3 and the
  implementation:

  1. Revoke erases the payload, unrevoke restores it; the action
     stays in the DAG, so peers can prove revocation and refuse to
     forward the payload.
  2. Cost: at least 1000, paid by the revoker; the target and its
     author lose half each (rule 3.b); a like also counts toward
     unrevoking.
  3. The author may revoke their own post for free (right to be
     forgotten), implemented but unstated.
  4. Revocation cancels the post's emission (rule 1.b, the new rule),
     which is the defense against farming.
  5. Revoke is meant for abuse, not disagreement, since it is
     expensive and dislikes exist for that.
\fi

...
and robust access.
On the other hand, they concentrate excessive power, controlling and
monetizing over private user data.
%
...
peer-to-peer content dissemination protocol that provides strong eventual
...
To conclude, we should keep in mind that our consensus algorithm is based on
...
verifies if all received blocks are consistent.

Because the protocol relies on tamper-proof DAGs, and because each peer self
verifies the blocks as they synchronize, the protocol becomes tolerant to
...
data can be detected on the first inconsistent block.

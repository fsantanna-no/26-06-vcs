...
in \code{branch-2} (in gray) is a dislike to member $c$ that affects his
...
As Figure~\ref{fig.hard} illustrates, a branch settles once it holds at
least 100 actions or spans 7 days between its oldest and newest actions.
Actions older than that window are frozen, and cannot be reorder.
If consensus would reorder frozen actions, then the merge is simply refused and
the two peers become incompatible, which is analogous to a hard fork in
Bitcoin.

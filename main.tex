...
\emph{Merkle Directed Acyclic Graph} (DAG) over Git~\cite{p2p.ipfs}.
...
appends (posts) and unions (syncs), replicas converge by construction with
strong eventual consistency~\cite{fed.matrix,p2p.byz}.
...
Table~\ref{tab.commands} lists the basic operations of \FC (\code{fc}), which
guide the discussion through concrete examples.
...
The next sequence of commands simulates a peer \emph{X} that
    ...
    (ii)  posts message \emph{``Hello World''}, and

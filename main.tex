...
fork points may date back to any point in history.
This allows for malicious members to forge long branches that would farm
...
\begin{lstlisting}[
    float=tbp,
    label={lst.discard},
    caption={Members $a$ and $b$ refute the farming attack of $c$.}
]
# PEER AB
$ fc /chat sync C                   # receives branch-1
$ fc /chat discard 7c1e2a9          # drops branch-1
7c1e2a9 ... (c, x, y posts)
$ fc /chat dislike 1000 member c    # from a
$ fc /chat dislike 1000 member c    # from b
$ fc /chat sync C                   # merges again
$ fc /chat list order
...                                 # branch-2
...                                 # branch-1: c fails, discarded
\end{lstlisting}
...
\code{branch-1} locally before casting the dislikes that ban $c$, making the
next merge to now reject \code{branch-1} for good.
%
As they synchronize, note that all honest peers will converge to the same state
that rejects \code{branch-1}, i.e., a single refutal is sufficient to propagate
to the rest of the network.

Therefore, a minority attack to farm new \reps is refutable because it is
bounded by the \reps accumulated in the common history.
%
However, it requires human intervention of at least one honest member.

\subsubsection{Hard Forks}
\label{sec.design.partitions.hardforks}

ultimo caso da alice ter feito sem querer e precisar do discard

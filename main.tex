...
same forked DAG (ln~11--14), now closed with a merge commit (hidden since it is
not an action).
...
\\
         & 1.b & old post &
Consolidated post grants \sreps{+}{1K} to member, after 24h, at most once a
day.
\\
\hline
Expense  & 2   & new post &
New post charges \sreps{-}{500} to member temporarily (except begs), refunded
within 12h.
\\
\hline
Transfer & 3.a & tax      &
...
In \FC, members spend tokens named \reps to post and rate content in forums:
    a \code{post} initially penalizes members (against excess), until it
    consolidates and counts positively (towards authoring);
    a \code{like} or \code{dislike} rates members and content (towards
    quality);
    a \code{revoke} or \code{unrevoke} toggles content removal (against abuse).
%
...
\item [\S\ref{sec.design.sec}]
    ...
    how replicas converge, and when they stop.
\end{description}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{monotonic.png}
\caption{
...
\begin{figure}[t]
\centering
\includegraphics[width=0.75\columnwidth]{hard.png}
\caption{

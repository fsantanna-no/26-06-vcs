...
In this work, we adapt the idea of scarce resources to eradicate Sybils and
reach consensus, but now in the context of public forums.
...
Naturally, in permissionless forums, we cannot rely on the author's timestamps
...
Without moderation, permissionless forums are impractical due to Sybils abusing
the system.
For instance, it should take seconds to generate thousands of fake identities
and SPAM millions of messages.
...
positively;
a \code{(dis)like} helps subscribers to distinguish content amid excess;
a \code{(un)revoke} erases content when crossing a threshold.
...
systems~\cite{rep.pseudonyms,p2p.sybil,p2p.bitcoin}:
    ...
    how to decentralize power over the time?
%
To address bootstrapping, we propose to designate explicit \emph{pioneers} with
initial \reps when creating chains.
The pioneers shape the evolution of chains with the first posts and likes,
while they gradually transfer \reps to new members, making decentralization
possible.
...
\begin{verbatim}
 # PEER I
 $ ssh-keygen alice     # self-gen keypair
 $ ssh-keygen bob       # self-gen keypair
 $ fc /chat init --pionneer=alice --pioneer=bob
...
Note that identities are cheap self-generated keypairs,
%
Therefore


For us, permissionlees is not that any user can...,
...
How the first member 
To welcome 
On the one hand


Note that the pioneers rule~\code{1.a} solves the
